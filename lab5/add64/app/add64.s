...
endless
		...
		; read 32 bit input from switch 0-31
		LDR R2, =ADDR_DIP_SWITCH_31_0
		LDR R3, [R2]
		
		; add 32 bit value to summation variables R0 and R1
		LDR R4, =0
		ADDS R0, R3
		ADCS R1, R4 ; adds carry
		
		; display sum (divided into lower and upper 32 btis) on the LCD display
		LDR R4, =ADDR_LCD_BIN
		STR R0, [R4] 
		STR R1, [R4, #4]
